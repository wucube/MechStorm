...
2. **Sprint 3 后段**：仅当首个真实主动召唤技能进入开发，并且目标、占格与最小技能流程已经稳定后，将召唤作为独立垂直切片实现。
3. **P3 天赋 / Trigger 阶段**：天赋触发召唤通过 Trigger 或 Ability 复用统一召唤入口，不直接修改单位注册表；若此前没有真实召唤玩法，则到首个天赋召唤需求出现时再补齐。
...
- Task 2.7 继续只实现轻量 `BattleActionLog + BattleSnapshot`，不提前建立 Command 体系。
- Sprint 5 / P2 引入 `BattleCommand` 时同步增加 `BattleCommandLog`，并用相同 `Sequence` 关联 Command、ActionResult 和关键 Snapshot。
